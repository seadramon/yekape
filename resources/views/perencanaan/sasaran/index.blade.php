        ...
        <div class="col-12 mb-md-5 mb-xl-10">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h3 class="card-title">Data Sasaran</h3>
                    <div class="card-toolbar">
						@if (in_array('add', json_decode(session('ACTION_MENU_' . auth()->user()->id))))
							<a href="{{route('perencanaan.sasaran.create')}}" class="btn btn-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Tambah Data</a>
						@endif
                    </div>
                </div>
...
<script type="text/javascript">
	...
	var KTDatatablesServerSide = function () {
	    ...
	    var initDatatable = function () {
	        dt = $("#tabel_master_driver").DataTable({
	            ...
	            order: [[0, 'desc']],
	            stateSave: true,
	            ajax: "{{ route('perencanaan.sasaran.data') }}",
	            columns: [
	                {data: 'tahun', name: 'tahun', defaultContent: '-'},
	                {data: 'nama', name: 'nama', defaultContent: '-'},
	                {data: 'misi.nama', name: 'misi.nama', defaultContent: '-'},
	...
	$('body').on('click', '.delete', function () {
		if (confirm("Are You sure want to delete this Record?") == true) {
			...
			$.ajax({
				type:"post",
				url: "{{ route('perencanaan.sasaran.destroy') }}",
				data: {id : id, _token: "{{ csrf_token() }}"},
				success: function(res){
					if (res.result == 'success') {
						flasher.success("Data has been deleted successfully!");

						$('#tabel_teacher').DataTable().ajax.url("{{ route('perencanaan.sasaran.data') }}").load();
					}
				}
			});
		}

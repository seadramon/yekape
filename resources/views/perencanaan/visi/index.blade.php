            ...
            <div class="card shadow-sm">
                <div class="card-header">
                    ...
                    <div class="card-toolbar">
                        <a href="{{route('perencanaan.visi.create')}}" class="btn btn-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Tambah Data</a>
                    </div>
                </div>
...
<script type="text/javascript">
	...
	var KTDatatablesServerSide = function () {
	    ...
	    var initDatatable = function () {
	        dt = $("#tabel_master_driver").DataTable({
	            ...
	            order: [[0, 'desc']],
	            stateSave: true,
	            ajax: "{{ route('perencanaan.visi.data') }}",
	            columns: [
	...
	$('body').on('click', '.delete', function () {
		if (confirm("Are You sure want to delete this Record?") == true) {
			...
			$.ajax({
				type:"post",
				url: "{{ route('perencanaan.visi.destroy') }}",
				data: {id : id, _token: "{{ csrf_token() }}"},
				success: function(res){
					if (res.result == 'success') {
						flasher.success("Data has been deleted successfully!");

						$('#tabel_teacher').DataTable().ajax.url("{{ route('perencanaan.visi.data') }}").load();
					}
				}
			});
		}

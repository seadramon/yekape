            ...
            <div class="card shadow-sm">
                <div class="card-header">
                    ...
                    <div class="card-toolbar">
                        ...
                        <a href="{{route('spk.create')}}" class="btn btn-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Tambah Data</a>
						@endif
                    </div>
                </div>
...
<script type="text/javascript">
	    ...
	    var initDatatable = function () {
	        dt = $("#tabel_master_driver").DataTable({
	            ...
	            columns: [
	                ...
	                {data: 'serapan.nama', name: 'serapan.nama', defaultContent: '-'},
	                {data: 'kontraktor.nama', name: 'kontraktor.nama', defaultContent: '-'},
	...
	$('body').on('click', '.delete', function () {
		if (confirm("Are You sure want to delete this Record?") == true) {
			...
			$.ajax({
				type:"post",
				url: "{{ route('spk.destroy') }}",
				data: {id : id, _token: "{{ csrf_token() }}"},
				success: function(res){
					if (res.result == 'success') {
						flasher.success("Data has been deleted successfully!");

						$('#tabel_teacher').DataTable().ajax.url("{{ route('spk.data') }}").load();
					}
				}
			});
		}

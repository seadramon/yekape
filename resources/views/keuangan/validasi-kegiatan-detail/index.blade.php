        ...
        <div class="col-12 mb-md-5 mb-xl-10">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h3 class="card-title">Data Validasi Rincian Kegiatan</h3>
...
<script type="text/javascript">
	...
	var KTDatatablesServerSide = function () {
	    ...
	    var initDatatable = function () {
	        dt = $("#tabel_master_driver").DataTable({
	            ...
	            order: [[0, 'desc']],
	            stateSave: true,
	            ajax: "{{ route('keuangan.validasi-kegiatan-detail.data') }}",
	            columns: [
	...
	$('body').on('click', '.delete', function () {
		if (confirm("Are You sure want to delete this Record?") == true) {
			...
			$.ajax({
				type:"post",
				url: "{{ route('keuangan.validasi-kegiatan-detail.destroy') }}",
				data: {id : id, _token: "{{ csrf_token() }}"},
				success: function(res){
					if (res.result == 'success') {
						flasher.success("Data has been deleted successfully!");

						$('#tabel_teacher').DataTable().ajax.url("{{ route('keuangan.validasi-kegiatan-detail.data') }}").load();
					}
				}
			});
		}
